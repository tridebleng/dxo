...
# // Start
echo "$(date) Autokill Vmess MultiLogin for tridebleng Script V1.0 Stable";
...
do
    ...
    if [[ $jumlah_baris_log -gt $VMESS ]]; then
        ...
        echo "VMESS - $(date) - ${username} - Multi Login Detected - Killed at ${date}" >> /etc/tridebleng/autokill.log;
    fi
