...
cat /etc/tridebleng/xray-mini-utils/shadowsocks-backup.json | jq 'del(.inbounds[0].settings.clients[] | select(.email == "'${client}'"))' > /etc/xray-mini/shadowsocks.json;
rm -rf /etc/tridebleng/shadowsocks/${client};
